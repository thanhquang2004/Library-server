...
const { validateCreateLending, validateExtendLending } = require("../utils/validate");
...
async function returnBook(req, res) {
    ...
    return res.json({ bookLendingId: lending.bookLendingId, returnDate: lending.returnDate, status: lending.status });
}
...
async function extendLending(req, res) {
    ...
    try {
        const lending = await service.extendLending(req.params.id, req.body.newDueDate);
        if (!lending) return res.status(404).json({ error: "Lending not found" });
        return res.json({ bookLendingId: lending.bookLendingId, dueDate: lending.dueDate });
    } catch (err) {
        return res.status(400).json({ error: err.message });
    }
}
