...
const cancelReservation = async (req, res) => {
  ...
  try {
    ...
    if (reservation.error)
      return res.status(404).json({ error: reservation.error });
...
const completeReservation = async (req, res) => {
  ...
  try {
    const reservation = await BookReservation.completeReservation(
      req.params.id
    );
    if (reservation.error)
      return res.status(404).json({ error: reservation.error });
...
const getMyReservations = async (req, res) => {
  ...
  try {
    const reservations = await BookReservation.getUserReservations(
      userIdFromToken
    );
...
const deleteReservation = async (req, res) => {
  ...
  try {
    const reservation = await BookReservation.softDeleteReservation(
      req.params.id
    );
    if (reservation.error)
      return res.status(404).json({ error: reservation.error });
...
const hardDeleteReservation = async (req, res) => {
  ...
  try {
    const reservation = await BookReservation.hardDeleteReservation(
      req.params.id
    );
    if (reservation.error)
      return res.status(404).json({ error: reservation.error });
